...
使用者於 2026-07-29 以 v1.2.4 的 AppImage 於 Ubuntu 26.04 實機測試，分兩階段。

**第一階段（`.deb` 與 AppImage 並存）**
...
此階段**不足以證明本功能有效**：`.deb` 會把自己的 `.desktop` 安裝到 `/usr/share/applications/`，其中同樣帶有 `StartupWMClass=app`，正是 AppImage 視窗的 WM_CLASS。於是「本功能有效」與「本功能毫無作用、由 `.deb` 頂替」在該機器上外觀完全相同——與本文件開頭記載的失敗模式同類：症狀與成功長得一樣。

**第二階段（移除 `.deb`，只有 AppImage）— 決定性驗證**

移除 `.deb` 安裝的 aiterm 後，單獨執行 AppImage 並點選建立選單項目：**dock 圖示確實由齒輪變為 AITerm 的圖示。**

混淆因素已排除，本功能的核心行為成立。

這同時間接證明了驗證清單第 9 項（`$APPDIR/usr/share/icons/hicolor` 是否存在）：若該樹缺失，`copy_icons` 會複製不到任何檔案，選單項目雖仍寫入 `Icon=aiterm` 卻指向不存在的圖示，dock 必定維持齒輪。既然圖示確實改變，該樹存在且內容可用。
