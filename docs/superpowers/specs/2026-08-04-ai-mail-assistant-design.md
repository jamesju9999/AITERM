...
**而且「已輪詢過的信」比想像中窄很多：首次同步只抓最新 50 封，之後只抓比 `last_seen_uid` 新的，舊信永遠不會被補進來。** 見 `client.rs::plan_fetch_batches` 的 `FIRST_POLL_MAX_MESSAGES = 50` 與 `since_uid.is_none()` 分支。這個上限是為了修「首次同步把整個 INBOX 一次抓完 → 超時 → 游標永遠不推進 → 每輪重試同樣失敗、一封都存不進來」那個 bug 才加的，不是隨意設的數字。

後果不只影響第 4 期的搜尋，**現在就影響 Mail 列表本身**——使用者在清單裡看到的永遠只有最新 50 封加上之後的新信，沒有任何 UI 能往前翻。要解除這個限制必須另外設計「回補舊信」的機制，而設計時的關鍵取捨是：每抓一封信目前都會伴隨一次 AI 摘要＋分類呼叫，所以全量回補一個上萬封的信箱等於上萬次 LLM 呼叫，時間與費用都不可行。可行的方向是把「為了搜尋而存內文」和「為了分流新信而跑 AI」拆開——舊信只存內文（供搜尋用），不跑摘要與重要性判斷（舊信本來就不該觸發通知）。這個設計尚未進行，記錄於此以免下次重新發現。
