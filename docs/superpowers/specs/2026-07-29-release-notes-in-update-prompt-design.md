...
| 來源 | CR 數量 |
|---|---|
| GitHub release body 的 changelog 區塊 | **4** |
| `latest.json` 的 `notes` | **0** |

內容經正規化後逐位元組相符。少了 `body.replace("\r\n", "\n")`，兩行之間的 `\r` 會原樣進入 `notes`，再進入更新提示的 `white-space: pre-wrap` 元素。

該修正由程式碼審查依推論提出（「區塊在網頁編輯器改寫，CRLF 是正常輸入」），在提出後的第二次發版即被實際觸發。

### 第 5 項（【查看完整說明】）

v1.2.5 發版時無法驗證，因為該按鈕由 v1.2.5 的程式碼渲染，而收到 v1.2.5 提示的機器跑的是 v1.2.4。v1.2.6 發版時，跑 v1.2.5 的機器即可看到。**驗證結果待補。**
