...
版本閘門**必須先於其餘 header 的反序列化**。若直接用完整的 v1 `Envelope` 結構去解，一份改動過 header 結構的 v2 檔案（換掉 KDF、把 nonce 移進密文、把 `version` 寫成字串）會先失敗在 serde，使用者拿到的是「這不是匯出檔」而不是「請先更新 AITerm」——版本欄位形同虛設，而且失效的恰好是最需要它的情況。所以先用一個只含 `format` 與 `version` 的最小結構解一次；這兩個欄位是這個格式唯一承諾永遠不變的。`version` 以 `serde_json::Value` 讀取，認不得的表示法一律視為「比我們新」。

**匯入檔的 Argon2 參數必須設上界**，而且要在這一關就檢查。`m_cost` / `t_cost` / `p_cost` 直接來自檔案，argon2 的 `Params::new` 只擋到 `u32::MAX`，而 `hash_password_into` 內部是不可失敗配置——`m_cost: 4000000` 會真的去要 3.8 GiB，再大就 `handle_alloc_error` abort 整個行程，使用者連錯誤訊息都看不到。上界訂為 m_cost ≤ 1 GiB、t_cost ≤ 16、p_cost ≤ 4（我們自己寫出去的是 19 MiB / 2 / 1）。檢查放在版本閘門同一層，`db_check_import_file` 才擋得住——晚一步就等於沒擋，因為 UI 是在要求輸入 passphrase 之前呼叫它的。
...
1. 比對池中有相同 `id` → `Overwrite`，目標是該筆
2. 否則，比對池中有相同 `name`（trim + 忽略大小寫）→ `Overwrite`，目標是該筆
3. 否則 → `New`，目標是匯出檔裡的 id

比對名稱是為了處理「同事在他的機器上手動建了同名連線」的情況——id 不同但實際是同一筆，不比名稱的話會變成兩筆同名連線並存。

**比對池不是固定快照。** 它從現有連線出發，每處理完一筆**會被套用**的資料就把它的目標 id 與名稱併進去。若只比對匯入前的固定快照，同一份檔案裡的兩筆資料可以各自解析到同一個目標：套用時後者默默蓋掉前者，而 `ImportResult` 還會把兩筆都算成成功、`failures` 是空的——那正好摧毀了「不做 rollback、改用逐筆回報」所倚賴的誠實性。

因此每個目標只能被認領一次，第二筆指向同一目標者判為第三種狀態 `Duplicate`：預覽中以灰字標示「與檔案中前一筆重複，將略過」、勾選框 disabled，套用時直接跳過，不計入 `added` 或 `overwritten`。被判為 `Duplicate` 的那筆**不會**進入比對池——它從未被套用，把它的名稱放進去會讓後面同名的一筆比對到一個匯入後根本不存在的名稱，於是也被誤判成重複而漏掉。
