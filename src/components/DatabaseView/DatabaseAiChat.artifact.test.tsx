...
describe("DatabaseAiChat artifact wiring", () => {
  ...
  it("mounts inside an ArtifactSplit so artifacts have somewhere to render", () => {
    const { container } = render(
      <DatabaseAiChat connectionId="c1" schema="" />,
    );
  ...
  it("closes the document panel when switching to a history entry that has none", async () => {
    localStorage.setItem("aiterm-db-chat-history-c1", JSON.stringify([
    ...
    await act(async () => {
      ({ container } = render(<DatabaseAiChat connectionId="c1" schema="" />));
    });
