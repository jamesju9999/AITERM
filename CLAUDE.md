...
- **`aiterm-elevated-host` is the same sidecar pattern, Windows-only**: registered in `externalBin` only on `tauri.windows.conf.json` (not the base conf or macOS/Linux), since the elevated-PTY feature only exists on Windows. Run `scripts/setup-elevated-host-win.ps1` first (compiles `aiterm-elevated-host` from source via `cargo build --release -p aiterm-elevated-host` and copies it to `src-tauri/binaries/aiterm-elevated-host-x86_64-pc-windows-msvc.exe`) — same compile-time `build.rs` validation gotcha as `uv`/DB2 applies here too.
